  ...
  return (
    <ol className={`relative flex flex-col gap-3 before:absolute before:bottom-5 before:left-[15px] before:top-5 before:w-px ${t.paso.linea}`}>
      {items.map((item, index) => {
        const texto = normalizarItem(item);
        return (
          <li key={`${index}-${texto}`} className={`relative flex gap-4 rounded-xl border p-4 pl-3 ${t.paso.caja}`}>
            ...
            <p className="pt-1 text-sm leading-relaxed text-tivit-ink/75">{texto}</p>
          </li>
        );
      })}
    </ol>
      ...
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-3 text-sm font-medium leading-relaxed text-tivit-ink/80 sm:mt-3">{texto}</p>
          </div>
        );
      })}
    </div>
