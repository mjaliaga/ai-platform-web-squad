...
// TODO: extraer a componentes — PortfolioGrid y ProjectForm ya extraídos (reduce god component 1229→~870 líneas).
// Queda pendiente extraer lógica de edición (BacklogFields/OtherStagesFields) y helpers getCamposPendientes/cleanForm a lib.
...
export function Projects() {
  ...
  // Filtros sincronizados con URL para deep-link y persistencia en refresh
  ...
  const [filterTipoProyecto, setFilterTipoProyecto] = useState(() => searchParams.get("tipo") || "all");

  // Sync filtros ↔ URL (deep-link). Actualiza URL cuando cambian filtros en vista lista.
  useEffect(() => {
    if (id) return; // solo en listado
    const next = new URLSearchParams();
    if (searchQuery) next.set("q", searchQuery);
    if (filterSector !== "all") next.set("sector", filterSector);
    if (filterStage !== "all") next.set("stage", filterStage);
    if (filterTipoProyecto !== "all") next.set("tipo", filterTipoProyecto);
    const cur = searchParams.toString();
    const nxt = next.toString();
    if (cur !== nxt) setSearchParams(next, { replace: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery, filterSector, filterStage, filterTipoProyecto, id]);
        ...
        <div className="mt-4 rounded-2xl border border-black/5 bg-white p-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 flex-wrap">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-xl sm:text-2xl font-bold text-tivit-ink">{current.name}</h1>
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-tivit-red-dark">Portafolio</h1>
      ...
      {/* Portafolio grids + filtros extraídos a PortfolioGrid.jsx (mantiene responsividad) */}
      {!id && (
        <PortfolioGrid
          projects={projects}
          allItems={allItems}
      ...
      {/* Grillas ahora en PortfolioGrid.jsx — evita duplicar 80+ líneas de Tailwind */}
      {!loading && allItems.length === 0 ? null : null}
    </div>
  );
}
