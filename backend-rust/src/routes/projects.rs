...
use crate::models::{Claims, Project, ProjectMemberWithUser, ProjectWithStats};
...
fn can_transition(
    ...
    if !allowed_next.contains(&next) {
        ...
        if !(current == "Evaluación técnica"
            && next == "Proyecto"
            && data.get("complejidad").and_then(|v| v.as_str()) == Some("Baja"))
        {
            return Err(crate::validation::error_response(
                axum::http::StatusCode::BAD_REQUEST,
                format!(
                    "Transición no permitida de '{}' a '{}'. Permitidas: {}",
                    current,
                    next,
                    allowed_next.join(", ")
                ),
    ...
    match current {
        "Backlog" => {
            ...
            if !desc_ok {
                return Err(crate::validation::error_response(
                    ...
                    "Backlog: falta descripcion_problema/valor_esperado".to_string(),
                ));
            }
        }
        "Evaluación técnica" => {
            if next == "PoC" {
                let has_lider = data
                    .get("lider_tecnico")
                    .or_else(|| data.get("ingeniero_encargado"))
                    .and_then(|v| v.as_str())
                    .map(|s| !s.trim().is_empty())
                    .unwrap_or(false);
                let has_tshirt = data
                    .get("tshirt")
                    .and_then(|v| v.as_str())
                    .map(|s| ["S", "M", "L", "XL"].contains(&s))
                    .unwrap_or(false);
                if !has_lider || !has_tshirt {
                    return Err(crate::validation::error_response(
                        StatusCode::BAD_REQUEST,
                        "Evaluación: falta ingeniero encargado o estimación tshirt (S/M/L/XL)"
                            .to_string(),
                    ));
                }
            }
            if next == "Proyecto" {
                ...
                if comp != "Baja" {
                    return Err(crate::validation::error_response(
                        StatusCode::BAD_REQUEST,
                        "Solo complejidad Baja puede saltar PoC directo a Proyecto".to_string(),
                    ));
                }
            }
        }
        "PoC" => {
            ...
            if !go {
                return Err(crate::validation::error_response(
                    StatusCode::BAD_REQUEST,
                    "PoC: requiere decisión Go".to_string(),
                ));
            }
        }
        "Proyecto" => {
            ...
            if !qa || !uat {
                return Err(crate::validation::error_response(
                    StatusCode::BAD_REQUEST,
                    "Proyecto: requiere qa_aprobado y uat_aprobado".to_string(),
                ));
            }
        }
        _ => {}
...
async fn assert_project_access(
    ...
    let member: Option<(String,)> =
        sqlx::query_as("SELECT user_id FROM project_members WHERE project_id = ? AND user_id = ?")
            .bind(project_id)
            .bind(&claims.sub)
            .fetch_optional(db)
            .await
    ...
    Err(crate::validation::error_response(
        ...
        "No tienes acceso a este proyecto. Solo miembros del proyecto pueden ver esta información."
            .to_string(),
    ))
}
...
pub async fn list_projects(
    ...
    if let Some(ref t) = tipo_filter {
        if t != "interno" && t != "comercial" {
            return Err(error_response(
                StatusCode::BAD_REQUEST,
                "tipo_proyecto debe ser 'interno' o 'comercial'".to_string(),
            ));
        }
    }
    ...
    let rows: Vec<Project> = {
        ...
        if claims.role == "admin" {
            let mut where_parts = vec![
                ...
                "slug IS NULL".to_string(),
            ];
            let mut binds: Vec<String> = vec![];
            if let Some(cat) = categoria_filter.clone() {
                where_parts.push("categoria = ?".to_string());
                binds.push(cat);
            }
            if let Some(st) = stage_filter.clone() {
                where_parts.push("stage = ?".to_string());
                binds.push(st);
            }
            if let Some(tp) = tipo_filter.clone() {
                where_parts.push("tipo_proyecto = ?".to_string());
                binds.push(tp);
            }
            let where_sql = where_parts.join(" AND ");
            let sql = format!(
                "SELECT {} FROM projects WHERE {} ORDER BY name LIMIT ? OFFSET ?",
                base_cols, where_sql
            );
            ...
            let mut where_parts_member = vec![
                "p.status = 'active'".to_string(),
                "p.deleted_at IS NULL".to_string(),
                "p.slug IS NULL".to_string(),
            ];
            let mut binds_member: Vec<String> = vec![];
            if let Some(cat) = categoria_filter.clone() {
                where_parts_member.push("p.categoria = ?".to_string());
                binds_member.push(cat);
            }
            if let Some(st) = stage_filter.clone() {
                where_parts_member.push("p.stage = ?".to_string());
                binds_member.push(st);
            }
            if let Some(tp) = tipo_filter.clone() {
                where_parts_member.push("p.tipo_proyecto = ?".to_string());
                binds_member.push(tp);
            }
    ...
    let total: i64 = {
        if claims.role == "admin" {
            ...
            let mut binds: Vec<String> = vec![];
            if let Some(cat) = categoria_filter.clone() {
                where_parts.push("p.categoria = ?".to_string());
                binds.push(cat);
            }
            if let Some(st) = stage_filter.clone() {
                where_parts.push("p.stage = ?".to_string());
                binds.push(st);
            }
            if let Some(tp) = tipo_filter.clone() {
                where_parts.push("p.tipo_proyecto = ?".to_string());
                binds.push(tp);
            }
...
pub async fn create_project(
    ...
    tracing::info!(
        "create_project called by {} with sponsor_id={:?} stage={:?}",
        claims.sub,
        payload.sponsor_id,
        payload.stage
    );
    ...
    tracing::info!(
        "create payload sponsor_id={:?} stage={:?} tipo={:?}",
        payload.sponsor_id,
        payload.stage,
        payload.tipo_proyecto
    );
    ...
    let mut final_portfolio_data = payload
        .portfolio_data
        .clone()
        .unwrap_or_else(|| serde_json::json!({}));
    ...
    if final_stage == "Backlog" && payload.categoria != "Proyecto" {
        match payload.categoria.as_str() {
            "Backlog de Propuestas Internas" => {
                final_stage = "Backlog".to_string();
                final_tipo = Some("interno".to_string());
            }
            "Backlog de Propuestas Comerciales" => {
                final_stage = "Backlog".to_string();
                final_tipo = Some("comercial".to_string());
            }
    ...
    let portfolio_data_str =
        serde_json::to_string(&final_portfolio_data).unwrap_or_else(|_| "{}".to_string());
    sqlx::query(
    ...
    if payload.members.as_ref().is_none_or(|m| m.is_empty()) {
        sqlx::query(
    ...
    Ok((
        StatusCode::CREATED,
        Json(build_project_with_stats(&state.db, row).await?),
    ))
}
    ...
    if let Some(new_stage) = &payload.stage {
        ...
        if new_stage != existing.stage.as_deref().unwrap_or("Backlog") {
            ...
            let effective_sponsor = payload
                ...
                .or_else(|| {
                    existing.sponsor_id.as_ref().and_then(|s| {
                        let s = s.trim();
                        if s.is_empty() {
                            None
                        } else {
            ...
            can_transition(
                existing.stage.as_deref().unwrap_or("Backlog"),
                new_stage,
                &claims,
                &effective_data,
                &effective_sponsor,
...
pub async fn update_member_role(
    ...
    if result.rows_affected() == 0 {
        return Err(error_response(
            StatusCode::NOT_FOUND,
            "Miembro no encontrado en este proyecto".to_string(),
        ));
    }
...
pub async fn set_published(
    ...
    let result =
        sqlx::query("UPDATE projects SET published = ?, updated_at = datetime('now') WHERE id = ?")
            .bind(val)
            .bind(&id)
            .execute(&state.db)
            .await
    ...
    Ok(Json(
        serde_json::json!({ "ok": true, "published": body.published }),
    ))
}
...
pub async fn set_reservado(
    ...
    let result =
        sqlx::query("UPDATE projects SET reservado = ?, updated_at = datetime('now') WHERE id = ?")
            .bind(val)
            .bind(&id)
            .execute(&state.db)
            .await
    ...
    Ok(Json(
        serde_json::json!({ "ok": true, "reservado": body.reservado }),
    ))
}
...
pub fn public_router(state: Arc<AppState>) -> axum::Router {
    ...
    axum::Router::new()
        ...
        .route(
            ...
            get(public_get_project_by_slug),
        )
        .with_state(state)
}
