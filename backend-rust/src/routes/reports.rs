...
pub async fn get_project_reports(
    ...
    let velocity_rows = vq
        .fetch_all(&state.db)
        .await
        .map_err(|e| internal_error(&format!("db error: {e}")))?;
    let velocity: Vec<VelocityPoint> = velocity_rows
        .into_iter()
        .map(|(name, committed, completed)| VelocityPoint {
            sprint_name: name,
            committed,
            completed,
        })
    ...
    workload_sql
        .push_str(" GROUP BY u.id, u.name, u.avatar_color ORDER BY task_count DESC LIMIT 20");

    let mut wq = sqlx::query_as::<
        _,
        (
            String,
            String,
    ...
    let workload_rows = wq
        .fetch_all(&state.db)
        .await
        .map_err(|e| internal_error(&format!("db error: {e}")))?;
    let workload: Vec<WorkloadEntry> = workload_rows
        .into_iter()
        .map(
            |(uid, uname, color, count, sp, ip, td, rv, dn)| WorkloadEntry {
                user_id: uid,
                user_name: uname,
                avatar_color: color,
                task_count: count,
    ...
    let burndown_rows = bq
        .fetch_all(&state.db)
        .await
        .map_err(|e| internal_error(&format!("db error: {e}")))?;
    let total_tasks: i64 = if let Some(pid) = &project_id {
        sqlx::query_as::<_, (i64,)>(
            "SELECT COUNT(*) FROM tasks WHERE deleted_at IS NULL AND project_id = ?",
        )
        .bind(pid)
        .fetch_one(&state.db)
        .await
    ...
    let burndown: Vec<BurndownPoint> = burndown_rows
        .iter()
        .enumerate()
        .map(|(i, (_, remaining))| BurndownPoint {
            date: burndown_rows
                .get(i)
                .map(|(d, _)| d.clone())
                .unwrap_or_default(),
    ...
    let mut sq =
        sqlx::query_as::<_, (i64, Option<i64>, Option<i64>, Option<i64>, Option<i64>)>(summary_sql);
    if let Some(pid) = &project_id {
        sq = sq.bind(pid);
    }
    let (total, done, in_progress, total_sp, done_sp) = sq
        .fetch_one(&state.db)
        .await
        .map_err(|e| internal_error(&format!("db error: {e}")))?;
